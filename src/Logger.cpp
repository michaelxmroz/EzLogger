...
void Logger::Logging_Helpers::MergeFinalMessage(const EzString& prefix, const char* level, const EzString& message, const EzString& postfix, EzString& finalMessageOut)
{
    finalMessageOut = string_format("%s%s: %s%s", prefix.c_str(), level, message.c_str(), postfix.c_str());
}
